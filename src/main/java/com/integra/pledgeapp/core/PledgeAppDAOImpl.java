...
//	}
	...
	public JSONArray getEmployeeList(String value, String group, String company, String document) {
		...
		try {
			...
			String listType = value;
			total = session.createQuery(
					"Select count(*) from Employee_Master e where e.compositeKey.EMP_GROUP =:group AND e.compositeKey.EMP_COMPANY=:company AND e.STATUS=1");
...
//							+ " WHERE ( e.EMP_GROUP =:group AND e.EMP_COMPANY=:company)) AS e) AS e1 "+ whereCondition);
					"SELECT * FROM (SELECT eid,ename,company,IFNULL(sdd.sign_order,0) AS empstatus,"
							...
							+ "FROM (SELECT e.EMP_ID AS eid, e.EMP_NAME AS ename, e.EMP_COMPANY AS company FROM EMPLOYEE_MASTER e WHERE (e.EMP_GROUP =:group AND e.EMP_COMPANY=:company AND e.STATUS='1')) a1 "
...
//		boolean res = false;
	...
	public JSONObject getDocInfoInEAM(String docCode) {
		...
		try {
			...
			@SuppressWarnings("unchecked")
			List<Employee_action_master> res = query.list();
...
//			}
	...
	public JSONObject bcstatewisesigningstatus(JSONObject input) {
		...
		try {
			...
			String hql = "SELECT bank,state,district,bcid,bcname,bcmobile,docCode,signstatus,signdate,bcaDataUploadedOn,bcaActiveStatus,PAN FROM \n\r"
					+ "(SELECT bank,state,district,bcid,bcname,bcmobile,docCode,CASE WHEN signstatus=0 THEN 'not signed'WHEN signstatus=1 THEN 'BC signed' WHEN signstatus=2 THEN 'BC & Witness signed'\r\n"
					...
					+ "(SELECT CASE WHEN em.STATUS=0 THEN 'Blocked' WHEN em.STATUS=1 THEN 'Active' WHEN em.STATUS=2 THEN 'Removed' ELSE 'check' END) AS bcaActiveStatus,\r\n"
					+ "em.EMP_COMPANY,\r\n"
					...
					+ "FROM Employee_Master em LEFT JOIN Sign_Doc_Details sdd ON sdd.EMPID=em.EMP_ID WHERE em.`EMP_COMPANY` ='i25BCA' AND em.EMP_ID NOT LIKE '999999%'";
			...
			if(input.getString("group").equalsIgnoreCase("All")) {
				String hqlExtened=")as t1)t2  ORDER BY bank,state,district,signstatus";
				hql=hql+hqlExtened;
				 query = session.createSQLQuery(hql);
				
			}
			else {
				String hqlExtened="AND em.`EMP_GROUP`=:empGrp)as t1)t2  ORDER BY bank,state,district,signstatus";
				hql=hql+hqlExtened;
				 query = session.createSQLQuery(hql);
				query.setParameter("empGrp", input.getString("group"));
			}
			...
			while (itr.hasNext()) {
				...
				jsobj.put("docCode", String.valueOf(obj[6]));
				jsobj.put("signstatus", String.valueOf(obj[7]));
				jsobj.put("signdate", String.valueOf(obj[8]));
				jsobj.put("bcaDataUploadedOn", String.valueOf(obj[9]));
				jsobj.put("bcaActiveStatus", String.valueOf(obj[10]));
				jsobj.put("PAN", String.valueOf(obj[11]));
...
//			
//			String hql1 = "SELECT bank,state,bcid,\r\n"
//					+ "(SELECT emp_name FROM employee_master em WHERE em.emp_id = bcid AND emp_group=bank) AS empname,\r\n"
//					+ "(SELECT `EMP_PHONE` FROM employee_master em WHERE em.emp_id  = bcid AND emp_group=bank) AS empmobile,\r\n"
//					+ "COUNT( *) rejection_count,COUNT( *)*50 rejection_charges FROM (	\r\n"
//					+ " SELECT bank,state,bcid FROM \r\n"
...
//					+ "(SELECT CASE WHEN JSON_VALID(em.ADDITIONAL_DATA) THEN JSON_UNQUOTE(em.ADDITIONAL_DATA->'$.state')ELSE NULL END )AS state\r\n"
//					+ "FROM Employee_Master em  INNER JOIN sign_rejected_info sdd ON em.EMP_ID=sdd.EMP_ID  WHERE  em.EMP_ID=sdd.EMP_ID AND em.emp_id NOT LIKE '9999999%'\r\n"
//					+ " AND em.`EMP_GROUP`=:empGrp ) AS t1)t2 GROUP BY bank,bcid,state";
//
//			
			...
			if(input.getString("group").equalsIgnoreCase("All")) {
				String hqlExtened=") AS t1)t2 GROUP BY bank,bcid,state";
				hql=hql+hqlExtened;
				 query = session.createSQLQuery(hql);
				
			}
			else {
				String hqlExtened=" AND em.`EMP_GROUP`=:empGrp ) AS t1)t2 GROUP BY bank,bcid,state";
				hql=hql+hqlExtened;
				 query = session.createSQLQuery(hql);
				query.setParameter("empGrp", input.getString("group"));
			}
